...
/*
		  for(int p=0;p<58;p++) {
			  long code=values[p];
			  if(Arrays.asList(table).contains(code)==false) {//If not in table, add
			  String word=keys[p];
			  table[p]=(word);
			  tableloc[p]=(code);
			  probe=-1;
			  ...
			  while((probe!=-1&&probe!=code)) { //continue untill entire table visited
				  if((Arrays.asList(table).contains(code)==false)) {
				 String word=keys[p];
				 table[p]=(word);
				 tableloc[p]=(code);
			     probe=-1;
...
for(int s=0;s<58;s++) {
	System.out.println(table[s]+"\t"+ tableloc[s]);
}
...
/*
		for(int o=0;o<=106;o++) {
			//linear probe
				long code1=values[o];
				  if(Arrays.asList(table).contains(code1)==false) {//If not in table, add
				String word=keys[o];
				table[o]=(word);
				tableloc[o]=(code1);
				 probe1=-1;
				...
				while((probe1!=-1&&probe1!=code1)) { //continue untill entire table visited
				  if((Arrays.asList(table).contains(code1)==false)) {
				  String word=keys[o];
				  table[o]=(word);
				  tableloc[o]=(code1);
				    probe1=-1;
